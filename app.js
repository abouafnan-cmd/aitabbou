...
function refreshCurrentView() {
    ...
    else if(activeSection === 'reports') viewReport(); // Updated reporting
...
/* ================== المراقبة المستمرة (تم الإصلاح الشامل) ================== */
function loadGrades() {
...
function viewReport() {
    ...
    // ----------- 2. تقرير الفروض المصغرة وتصحيح الدفاتر -----------
...
/* ================== دوال مساعدة للإدارة والمكونات (تم الحفاظ عليها) ================== */
...
function addClass() { const name = document.getElementById('newClassName').value.trim(); if (!name) return; if (db.classes.includes(name)) return alert("موجود مسبقاً!"); db.classes.push(name); db.students[name] = []; document.getElementById('newClassName').value = ""; init(); saveToCloud(true); }
function deleteClass() { const selClass = document.getElementById('manageClassSelect').value; if (!selClass) return; if (confirm(`حذف قسم "${selClass}" بالكامل؟`)) { db.classes = db.classes.filter(c => c !== selClass); delete db.students[selClass]; init(); document.getElementById('studentManagementArea').style.display = 'none'; saveToCloud(true); } }
