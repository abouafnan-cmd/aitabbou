...
function init() {
    ...
    // تهيئة الشهر الحالي لتبويب التقارير
...
/* ================== خريطة المراقبة (48 مقعداً ومكتب بالأسفل) ================== */
function loadSeatingMap() {
    ...
    if (!date || !cls || !db.students[cls] || db.students[cls].length === 0) { container.style.display = 'none'; msg.style.display = 'block'; return; }
    ...
    while (db.seatingMaps[date][cls].length < totalSeats) { db.seatingMaps[date][cls].push({ student: "", isEmpty: false, phone: 0, warnings: 0 }); }
...
function autoSaveTracking() {
    ...
    if(db.sessionMeta && db.sessionMeta[date] && db.sessionMeta[date][cls]) { sessionType = db.sessionMeta[date][cls]; }
    ...
    db.students[cls].forEach((student, i) => { 
        ...
        if(!disablePrep && prepNode) prepVal = prepNode.value;
...
/* ================== الفروض المصغرة (تم إصلاحها وإضافة قائمة منسدلة) ================== */
function loadMiniExams() {
    ...
    if (!date || !cls || !type || !db.students[cls] || db.students[cls].length === 0) {
        return list.innerHTML = '<p style="text-align: center; color: var(--text-muted); padding: 20px;">المرجو اختيار التاريخ، القسم، ونوع الفرض.</p>';
    }
    ...
    html += `<button class="btn-primary" style="width: 100%; padding: 15px; font-size: 1.1rem; margin-top: 10px;" onclick="saveMiniExams()">💾 حفظ الفروض المصغرة</button>`;
    list.innerHTML = html;
}
...
function saveMiniExams() {
    ...
    const cls = document.getElementById('miniExamsClassSelect').value;
    const type = document.getElementById('miniExamTypeSelect').value;
    if (!date || !cls || !type) return alert("المرجو إكمال جميع الاختيارات (التاريخ، القسم، نوع الفرض).");
    ...
    db.students[cls].forEach((student, index) => {
        ...
        if(node) db.miniExams[date][cls][type][student] = node.value;
    });
    saveToCloud(true);
    showToast("تم حفظ الفروض المصغرة بنجاح!");
}

/* ================== التقارير الشاملة واليومية والشهرية (محرك محدث) ================== */
function toggleReportInputs() {
    ...
    if (rType === 'tracking' || rType === 'miniExams') {
        ...
        if (tFrame === 'daily') {
            dInput.style.display = 'block'; mInput.style.display = 'none';
        } else if (tFrame === 'monthly') {
            dInput.style.display = 'none'; mInput.style.display = 'block';
        } else {
            dInput.style.display = 'none'; mInput.style.display = 'none';
        }
    } else {
    ...
    viewReport(); // تحديث فوري عند التغيير
}
...
function viewReport() {
    ...
    if (reportType === 'tracking') {
        ...
        if(tFrame === 'daily') {
            // عرض اليومي العادي (مثل النسخة السابقة)
            ...
            // عرض شهري أو شامل (جدول إحصائيات)
            let stats = {};
    ...
    // ----------- 2. تقرير الفروض المصغرة -----------
        ...
        if (tFrame === 'daily') {
            ...
            reportTitle = `تقرير الفروض المصغرة: ${selClass} (${date})`;
        } else if (tFrame === 'monthly') {
            const month = document.getElementById('reportMonth').value;
            if(month) Object.keys(db.miniExams).forEach(d => { if(d.startsWith(month) && db.miniExams[d][selClass]) datesToProcess.push(d); });
            reportTitle = `التقرير الشهري للفروض المصغرة: ${selClass} (${month})`;
        } else {
            Object.keys(db.miniExams).forEach(d => { if(db.miniExams[d][selClass]) datesToProcess.push(d); });
            reportTitle = `التقرير الشامل للفروض المصغرة: ${selClass}`;
        }

        if (datesToProcess.length === 0) { 
            display.innerHTML = '<p style="text-align:center; color:var(--danger);">لا توجد بيانات فروض مصغرة محفوظة لهذه الفترة.</p>';
            display.style.display = 'block'; actions.style.display = 'none'; return; 
        }

        datesToProcess.sort((a,b) => new Date(b) - new Date(a)); // الأحدث أولاً
        ...
        datesToProcess.forEach(date => {
            ...
            for(let type in dayExams) {
                ...
                html += `<tr style="background:var(--primary); color:white;"><th style="padding:8px; border:1px solid var(--border);">التلميذ</th><th style="padding:8px; border:1px solid var(--border);">التقييم</th></tr>`;
...
/* ================== دوال أخرى للملاحظات والإدارة (بدون تغيير) ================== */
function loadPortfolioStudents() { const cls = document.getElementById('portClassSelect').value; const select = document.getElementById('portStudentSelect'); const searchInput = document.getElementById('searchPortfolio'); document.getElementById('portfolioDisplay').style.display = 'none'; searchInput.value = ''; if(!cls) { select.style.display = 'none'; searchInput.style.display = 'none'; return; } select.innerHTML = '<option value="">-- اختر التلميذ --</option>'; db.students[cls].forEach((s, i) => select.innerHTML += `<option value="${s}">${i+1} - ${s}</option>`); select.style.display = 'block'; searchInput.style.display = 'block'; }
function viewPortfolio() { const cls = document.getElementById('portClassSelect').value; const student = document.getElementById('portStudentSelect').value; const display = document.getElementById('portfolioDisplay'); if(!cls || !student) { display.style.display = 'none'; return; } let totalAbsences = 0; let totalLates = 0; Object.values(db.records).forEach(dayRec => { if(dayRec[cls]) { const sRec = dayRec[cls].find(s => s.name === student); if(sRec && sRec.attendance === 'غائب') totalAbsences++; if(sRec && sRec.attendance === 'متأخر') totalLates++; } }); let bonus = (db.bonusPoints[cls] && db.bonusPoints[cls][student]) ? db.bonusPoints[cls][student] : 0; let discHtml = ''; Object.values(db.disciplinary).forEach(log => { if(log.class === cls && log.student === student) discHtml += `<div class="note-item">📅 ${log.date} - إجراء مسجل</div>`; }); let html = `<h2 style="color:var(--title-color); border-bottom:2px solid var(--accent); padding-bottom:10px;">🎓 بطاقة التلميذ(ة): ${student}</h2>`; html += `<div style="display:flex; gap:10px; flex-wrap:wrap; margin-bottom:20px;"><div style="background:var(--danger); color:white; padding:10px; border-radius:5px; flex-grow:1; text-align:center;">🚫 غيابات: ${totalAbsences}</div><div style="background:#f39c12; color:white; padding:10px; border-radius:5px; flex-grow:1; text-align:center;">⏳ تأخرات: ${totalLates}</div><div style="background:var(--accent); color:white; padding:10px; border-radius:5px; flex-grow:1; text-align:center;">⭐ نقط تحفيزية: ${bonus}</div></div>`; if(discHtml !== '') html += `<h4 style="color:var(--danger);">⚖️ الإجراءات التأديبية:</h4>${discHtml}`; html += `<h4 style="color:var(--title-color);">📝 الملاحظات السلوكية:</h4>`; let hasNotes = false; if (db.notes[cls] && db.notes[cls][student]) { Object.keys(db.notes[cls][student]).sort((a,b) => new Date(b) - new Date(a)).forEach(d => { const notes = db.notes[cls][student][d]; if(notes.length > 0) { hasNotes = true; const isPos = notes.includes("تقديم مشاركة متميزة") || notes.includes("استحقاق نقطة حسنة"); html += `<div class="note-item ${isPos ? 'positive':''}"><strong>${d}:</strong> ${notes.join('، ')}</div>`; } }); } if(!hasNotes) html += `<p style="color:var(--text-muted);">لا توجد ملاحظات مسجلة.</p>`; display.innerHTML = html; display.style.display = 'block'; }
function loadNoteStudents() { const cls = document.getElementById('noteClassSelect').value; const select = document.getElementById('noteStudentSelect'); const searchInput = document.getElementById('searchNotes'); document.getElementById('noteCheckboxesArea').style.display = 'none'; document.getElementById('noteHistoryArea').style.display = 'none'; document.getElementById('classNotesArea').style.display = 'none'; searchInput.value = ''; if (!cls || !db.students[cls]) { select.style.display = 'none'; searchInput.style.display = 'none'; return; } select.innerHTML = '<option value="">-- اختر التلميذ --</option>'; db.students[cls].forEach((s, i) => select.innerHTML += `<option value="${s}">${i+1} - ${s}</option>`); select.style.display = 'block'; searchInput.style.display = 'block'; }
function loadStudentHistory() { const date = document.getElementById('noteDate').value; const cls = document.getElementById('noteClassSelect').value; const student = document.getElementById('noteStudentSelect').value; const cbArea = document.getElementById('noteCheckboxesArea'); document.getElementById('classNotesArea').style.display = 'none'; if (!student) { cbArea.style.display = 'none'; document.getElementById('noteHistoryArea').style.display = 'none'; return; } document.querySelectorAll('#noteCheckboxesArea input[type="checkbox"]').forEach(cb => cb.checked = false); if (db.notes[cls] && db.notes[cls][student] && db.notes[cls][student][date]) { const todayNotes = db.notes[cls][student][date]; document.querySelectorAll('#noteCheckboxesArea input[type="checkbox"]').forEach(cb => { if (todayNotes.includes(cb.value)) cb.checked = true; }); } cbArea.style.display = 'block'; const histDiv = document.getElementById('noteHistory'); histDiv.innerHTML = ''; let hasHistory = false; if (db.notes[cls] && db.notes[cls][student]) { Object.keys(db.notes[cls][student]).sort((a,b) => new Date(b) - new Date(a)).forEach(d => { const notes = db.notes[cls][student][d]; if (notes.length > 0) { hasHistory = true; const isPos = notes.includes("تقديم مشاركة متميزة") || notes.includes("استحقاق نقطة حسنة"); histDiv.innerHTML += `<div class="note-item ${isPos ? 'positive' : ''}"><button class="btn-delete-note" title="حذف الملاحظة" onclick="deleteNoteRecord('${cls}', '${student}', '${d}')">🗑️</button><strong style="margin-right: 25px; color:var(--title-color);">📅 ${d}</strong><br><div style="margin-right: 25px; color:var(--text);">- ${notes.join('<br>- ')}</div></div>`; } }); } document.getElementById('studentArchiveTitle').innerText = `أرشيف ملاحظات: ${student}`; document.getElementById('noteHistoryArea').style.display = hasHistory ? 'block' : 'none'; }
function saveNotes() { const date = document.getElementById('noteDate').value; const cls = document.getElementById('noteClassSelect').value; const student = document.getElementById('noteStudentSelect').value; if (!date || !cls || !student) return alert("المرجو إكمال الاختيارات."); const selectedNotes = Array.from(document.querySelectorAll('#noteCheckboxesArea input[type="checkbox"]:checked')).map(cb => cb.value); if (!db.notes[cls]) db.notes[cls] = {}; if (!db.notes[cls][student]) db.notes[cls][student] = {}; db.notes[cls][student][date] = selectedNotes; saveToCloud(true); loadStudentHistory(); }
function deleteNoteRecord(cls, student, date) { if(confirm(`هل أنت متأكد من حذف الملاحظات المسجلة بتاريخ ${date}؟`)) { delete db.notes[cls][student][date]; saveToCloud(false, true); showToast("تم حذف الملاحظة بنجاح"); if(document.getElementById('classNotesArea').style.display === 'block') viewClassNotes(); else loadStudentHistory(); } }
function viewClassNotes() { const cls = document.getElementById('noteClassSelect').value; const classArea = document.getElementById('classNotesArea'); const classDiv = document.getElementById('classNotesDisplay'); if (!cls) return alert("المرجو اختيار القسم أولاً."); classDiv.innerHTML = ''; let hasClassNotes = false; if (db.notes[cls]) { for (let student in db.notes[cls]) { let studentHtml = `<div style="background: rgba(128,128,128,0.05); padding: 10px; border-radius: 5px; margin-bottom: 10px; border-right: 4px solid var(--primary);">`; studentHtml += `<h4 style="margin: 0 0 10px 0; color: var(--title-color);">👤 ${student}</h4>`; let hasStudentNotes = false; Object.keys(db.notes[cls][student]).sort((a,b) => new Date(b) - new Date(a)).forEach(d => { const notes = db.notes[cls][student][d]; if (notes.length > 0) { hasClassNotes = true; hasStudentNotes = true; const isPos = notes.includes("تقديم مشاركة متميزة") || notes.includes("استحقاق نقطة حسنة"); studentHtml += `<div class="note-item ${isPos ? 'positive' : ''}" style="margin-left: 10px; margin-right: 10px;"><button class="btn-delete-note" title="حذف الملاحظة" onclick="deleteNoteRecord('${cls}', '${student}', '${d}')">🗑️</button><strong style="margin-right: 25px; color:var(--title-color);">📅 ${d}</strong><br><div style="margin-right: 25px; color:var(--text);">- ${notes.join('<br>- ')}</div></div>`; } }); studentHtml += `</div>`; if (hasStudentNotes) classDiv.innerHTML += studentHtml; } } if (!hasClassNotes) classDiv.innerHTML = '<p style="text-align: center; color: var(--text-muted);">لا توجد ملاحظات مسجلة لتلاميذ هذا القسم.</p>'; classArea.style.display = 'block'; document.getElementById('noteCheckboxesArea').style.display = 'none'; document.getElementById('noteHistoryArea').style.display = 'none'; document.getElementById('noteStudentSelect').value = ""; }
...
function deleteLessonLog(id) { if(!confirm("هل أنت متأكد من حذف هذا السجل؟")) return; const date = document.getElementById('lessonDate').value; const cls = document.getElementById('lessonClassSelect').value; db.lessonLog[date][cls] = db.lessonLog[date][cls].filter(log => log.id !== id); saveToCloud(false); viewLessonLog(); }
